...
public class MantisIssueNode extends IssueNode<MantisIssue> {
    ...
    private static final IssueStatusProvider dummyStatusProvider
            = new IssueStatusProvider<MantisRepository, MantisIssue>() {

                @Override
                public IssueStatusProvider.Status getStatus(MantisIssue i) {
    ...
    private static final IssueNode.ChangesProvider dummyChangesProvider
            = new IssueNode.ChangesProvider<MantisIssue>() {
    ...
    public MantisIssueNode(MantisIssue issue) {
        super(
                issue.getMantisRepository().getInfo().getConnectorId(),
                issue.getMantisRepository().getInfo().getID(),
                issue,
                new MantisIssueProvider(),
                dummyStatusProvider,
                dummyChangesProvider
        );
    }
    ...
    private class ReflectionProperty<T> extends IssueNode<MantisIssue>.IssueProperty<T> {
        ...
        public ReflectionProperty(String name, String displayName, String shortDescription, Object instance, Class<T> valueType, String property) throws NoSuchMethodException {
            ...
            String accessorBase = property.substring(0, 1).toUpperCase()
